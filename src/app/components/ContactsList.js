...
class ContactList extends React.Component {
  ...
  _handleShowProfile(uuid, type, name) {
    this.props.dispatch(switchNav('Profile', {
      uuid,
      type,
      name,
    }));
  ...
  getHeader(section) {
    return (
      <TouchableOpacity style={styles.header} onPress={() => this._handleClickHeader(section)}>
        {
          section.isShow ? (
  ...
  getCell(item) {
    ...
    return (
      <ConvItem
        ...
        uuid={item.uuid}
        style={styles.cell}
        onPress={() => {this._handleShowProfile(item.uuid, item.type, item.name)}}
      />
    )
  }
  ...
  render() {
    if(this.props.friends.length === 0 && this.props.groups.length === 0) {
      return (
        <View><Text>暂无联系人, 快去交♂朋♂友吧</Text></View>
      )
    }
    return (
      <SectionList
  ...
  state => {
    ...
    return {
      friends: friends.map(f => ({
        ...
        name: usercache.getIn([f, 'nickname']) || usercache.getIn([f, 'username']),
        type: 'user'
      })),
      groups: groups.map(g => ({
        uuid: g.get('uuid'),
