  ...
  render() {
    let actions = (
      <div className="actions">
        <button onClick={() => this.props.dispatch(hideModal())}><i className="iconfont">&#xe70c;</i>取消</button>
        <button onClick={() => this._handleSaveGroupInfo()}><i className="iconfont">&#xe634;</i>保存</button>
      </div>
    )
...
module.exports = connect(
  state => {
    ...
    let selectedGroupIndex = state.getIn(['group', 'groups']).findIndex(g => g.get('uuid') === selectedGroupUUID);
    return {
      selectedGroupUUID,
